...
## 思路：
1. 设置两个变量，用一个变量检测是不是字母，不是就强转为数字

## 其中的一些方法
1. `getline(cin,s)` 用于读取整行的字符串
2. `int num=stoi(const string&  str, size_t* idx = 0, int base = 10);` 字符串转换为数字。 base为进制，idx为结束的位置
3. `to_string()` 为数字转换为字符串
...
int main(){
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(mid == "+") {
            ans = to_string(b) + mid + to_string(c) + "=" + to_string(b + c);
        } 
        else if(mid == "-") {
            ans = to_string(b) + mid + to_string(c) + "=" + to_string(b - c);
        } 
